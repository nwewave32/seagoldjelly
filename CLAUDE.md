...
- [x] Flutter + Flame 프로젝트 셋업, iOS 빌드/실행 확인
- [x] 금붕어 1마리가 화면에서 자연스럽게 헤엄 (wander 알고리즘, 화면 경계 처리)
- [x] 터치 시 금붕어가 손가락 쪽으로 다가오거나 흩어지는 반응
- [x] 기울기(가속도계)에 따라 물/금붕어가 반응
- [x] 기기에서 fps 측정 (devtools / overlay)
- **DoD:** 실기기에서 **60fps 유지** + "예쁘게 헤엄친다"는 주관적 합격. ❌ 미달 시 사람과 엔진 대안 상의.
  - ✅ 2026-06-28 실기기 profile 모드 60fps 확인(끊김 없음). 물/금붕어 터치·기울기 동작. "예쁘게 헤엄친다" 최종 합격은 사용자 확인.

### Phase 1 — 코어 루프
**목표:** "자기 전에 켜고 싶은가"를 본인이 매일 써보며 검증.
- [x] 호흡 프로그램 데이터 모델 + 4-7-8 1종 구현 (BreathingProgram/BreathingSession + 테스트)
- [x] 호흡 세션 중 금붕어 헤엄 속도가 호흡 페이즈(들숨/멈춤/날숨)에 싱크 (FishComponent.breathFactor)
- [x] 호흡 시각 가이드 (확장/수축 같은 미니멀한 연출) (BreathingGuide 원)
- [ ] 사운드 2~3종 재생 (루프, 끊김 없는 처리) — 라이선스 클린 오디오 파일 필요(§9)
- [~] 수면 타이머 + 종료 시 자동 페이드아웃 — 타이머·잠듦 연출(화면 어두워짐·금붕어 잠듦) 구현. 오디오 페이드아웃은 사운드 구현 시 연결(데모 8초, 실제 길이/설정은 Phase 4·무료 15분 §7)
...
- [ ] 먹이 주기 + 금붕어 반응
- [ ] 금붕어 예쁘게 하기 (Phase 0 코드 도형 플레이스홀더 → 스프라이트/디테일 비주얼, 감정 표현 반영). `fish_component.dart` render 교체 지점
